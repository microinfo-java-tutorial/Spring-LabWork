...
public class HibernateItemRepository implements ItemRepository {
	...
	public Collection<MusicItem> searchByArtistTitle(String keyword) {
	 ...
	 public void update(MusicItem itemToUpdate) {
		 Session s = getSessionFactory().getCurrentSession();
		 s.beginTransaction();
		 MusicItem ret = (MusicItem)s.get(MusicItem.class, itemToUpdate.getId());
		 ret.setTitle(itemToUpdate.getTitle());
		 ret.setPrice(itemToUpdate.getPrice());
		 s.update(ret);
		 s.getTransaction().commit();
	 }
